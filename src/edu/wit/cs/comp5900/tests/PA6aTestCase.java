...
import edu.wit.cs.comp5900.PA6a;
...
public class PA6aTestCase extends TestCase {
	...
	private void _testDigitCharToInt(char c1, char c2, int expected) {
		Integer result = null;
		try {
			result = PA6a.digitCharToInt(c1, c2);
		} catch (ExitException e) {}
		assertEquals(expected, (int) result);
	}
	...
	private void _testGetDigitInPlace(int num, int place, int expected) {
		Integer result = null;
		try {
			result = PA6a.getDigitInPlace(num, place);
		} catch (ExitException e) {}
		assertEquals(expected, (int) result);
	}
	...
	private void _testSumOfDoubleEvens(String s, int expected) {
		Integer result = null;
		try {
			result = PA6a.sumOfDoubleEvens(s);
		} catch (ExitException e) {}
		assertEquals(expected, (int) result);
	}
